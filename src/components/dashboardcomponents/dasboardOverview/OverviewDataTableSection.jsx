...
const tables = [
    ...
    { title: "Landlord", bgColor: "bg-orange-500" },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-5">
            {tables.map((table, tableIndex) => (
                <div key={tableIndex} className="bg-white">
                    <CardHeader className={`${table.bgColor} text-white rounded-t-lg border-2`}>
                        <div className="flex items-center justify-between">
                            <CardTitle className="text-lg font-semibold">{table.title}</CardTitle>
                            <Button variant="ghost" size="sm" className="text-white hover:bg-white/20 mt-2">
                                Sell All
                            </Button>
                        </div>
                    </CardHeader>
